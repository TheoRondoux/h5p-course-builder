...
require_login();
if (!is_siteadmin()) {
    h5plib_course_builder_redirect_error(get_string('noaccesstosettings', 'h5plib_course_builder'));
}

$context = context_system::instance();
$PAGE->set_context($context);
$PAGE->set_url(new moodle_url('/h5p/h5plib/course_builder/configuration.php'));
$PAGE->set_pagelayout('standard');
$PAGE->set_title(get_string('pluginname', 'h5plib_course_builder') . " " . $SITE->fullname);
$PAGE->set_heading(get_string('configtitle', 'h5plib_course_builder'));

h5plib_course_builder_display_logo();
$configForm = new \h5plib_course_builder\form\config_form();
$addTemplateForm = new \h5plib_course_builder\form\config_add_template_form();
$updateTemplateForm = new \h5plib_course_builder\form\config_update_template_form();
$deleteTemplateForm = new \h5plib_course_builder\form\config_delete_template_form();

if ($data = $configForm->get_data()) {

    $categoryName = "Course Builder";
    $dbCategory = $DB->get_record('course_categories', ["name" => $categoryName]);
    $categoryId = 0;
    if (!$dbCategory) {
        $newCategory = new stdClass();
        $newCategory->name = $categoryName;
        $newCategory->description = get_string('categorydescription', 'h5plib_course_builder');
    ...
    $newCourse->shortname = "coursebuilder";
    $newCourse->fullname = "Course Builder";
...
if ($data = $addTemplateForm->get_data()) {
    ...
    if (isset($data->available_templates)) {
        ...
        $templateCourseId = h5p_course_builder_get_template_course()->id;
        $addedTemplates = h5p_course_builder_get_added_templates();
        $availableTemplates = h5p_course_builder_get_available_templates($addedTemplates, $templateCourseId);
...
$configForm->display();
echo html_writer::tag('br', '');
echo html_writer::tag('h3', get_string('templatemanagementtitle', 'h5plib_course_builder'));

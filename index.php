...
$userPresentations =
        $DB->get_records_sql('SELECT mdl_hvp.id, mdl_hvp.name, mdl_hvp.timecreated, mdl_hvp.timemodified, mdl_user.firstname, mdl_user.lastname, mdl_h5plib_course_builder_pres.userid, mdl_h5plib_course_builder_pres.shared FROM mdl_hvp,mdl_h5plib_course_builder_pres, mdl_user WHERE mdl_hvp.id IN (SELECT presentationid FROM mdl_h5plib_course_builder_pres WHERE userid = ' .
                $USER->id . ') AND mdl_hvp.id = mdl_h5plib_course_builder_pres.presentationid AND mdl_h5plib_course_builder_pres.userid = mdl_user.id ORDER BY mdl_hvp.timemodified DESC');
$sharedPresentations =
...
echo html_writer::start_tag('center');
echo html_writer::tag('a', get_string('createnewpresentation', 'h5plib_course_builder'), h5plib_course_builder_get_create_btn_attributes());
echo html_writer::end_tag('center');
echo html_writer::tag('br', '');
...
if ($userPresentations && count($userPresentations) < 7) {
    ...
    echo html_writer::start_tag('center');
    echo html_writer::tag('a', get_string('showpresentations', 'h5plib_course_builder'), h5plib_course_builder_get_presentation_btn_attributes());
    echo html_writer::end_tag('center');
